...
user.DestroyIcon.argtypes = (wt.HICON,)
user.DestroyIcon.restype = wt.BOOL
...
user.RegisterClassExW.argtypes = (ctypes.POINTER(WndClassExw),)
user.RegisterClassExW.restype = wt.ATOM

user.UnregisterClassW.argtypes = (wt.LPCWSTR, wt.HINSTANCE)
user.UnregisterClassW.restype = wt.BOOL
...
user.UpdateWindow.argtypes = (wt.HWND,)
user.UpdateWindow.restype = wt.BOOL

user.DestroyWindow.argtypes = (wt.HWND,)
user.DestroyWindow.restype = wt.BOOL

shell32.Shell_NotifyIconW.argtypes = (wt.DWORD, ctypes.POINTER(NotifyIconDataW))
shell32.Shell_NotifyIconW.restype = wt.BOOL

user.GetMessageW.argtypes = (ctypes.POINTER(Msg), wt.HWND, wt.UINT, wt.UINT)
user.GetMessageW.restype = wt.INT

# user32.PeekMessageW.argtypes = (ctypes.POINTER(Msg), wt.HWND, wt.UINT, wt.UINT, wt.UINT)
# user32.PeekMessageW.restype = wt.BOOL

user.TranslateMessage.argtypes = (ctypes.POINTER(Msg),)
user.TranslateMessage.restype = wt.BOOL

user.DispatchMessageW.argtypes = (ctypes.POINTER(Msg),)
...
user.AttachThreadInput.restype = wt.BOOL
...
class _App():
    ...
    def _register_win_cls(self):
        ...
        cls_atom = user.RegisterClassExW(ctypes.byref(win_cls))
        if not cls_atom:
    ...
    def _unregister_win_cls(self):
        ...
        """
        if self._win_cls:
            user.UnregisterClassW(self._win_cls.lpszClassName, self._hinstance)
            self._win_cls = None
            self._cls_atom = None
    ...
    def _run_event_loop(self):
        ...
        filter_max = 0
        # print("starting")
        while (rv := user.GetMessageW(msg_p, hwnd, filter_min, filter_max)) != 0:
            # print("pumped")
            if rv == -1:
            ...
            if msg.message == MsgValue.SHUTDOWN_THREAD:
                # print("shutting down")
                break
            user.TranslateMessage(msg_p)
            user.DispatchMessageW(msg_p)
    ...
    def _run(self, main_th_id: int):
        ...
        user.AttachThreadInput(main_th_id, child_th_id, True)
        self._init()
        try:
            self._run_event_loop()
        finally:
            self._deinit()
            user.AttachThreadInput(main_th_id, child_th_id, False)
    ...
    def stop(self):
        ...
        """
        if self._hwnd:
            user.PostMessageW(self._hwnd, MsgValue.SHUTDOWN_THREAD, 0, 0)
            if self._thread:
                self._thread.join()
                self._thread = None
