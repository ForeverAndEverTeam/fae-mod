...
user.GetWindowTextLengthW.restype = wt.INT

user.GetWindowTextW.argtypes = (wt.HWND, wt.LPWSTR, wt.INT)
user.GetWindowTextW.restype = wt.INT

user.EnumWindows.argtypes = (WNDENUMPROC, wt.LPARAM)
user.EnumWindows.restype = wt.BOOL

user.GetWindowRect.argtypes = (wt.HWND, wt.LPRECT)
user.GetWindowRect.restype = wt.BOOL

user.FlashWindowEx.argtypes = (ctypes.POINTER(FlashWInfo),)
user.FlashWindowEx.restype = wt.BOOL
...
def get_hwnd_by_title(title: str) -> int|None:
    ...
    def callback(hwnd: int, lparam: int) -> bool:
        ...
        if user.IsWindowVisible(c_hwnd):
    ...
    user.EnumWindows(WNDENUMPROC(callback), wt.LPARAM(0))
    return pack.value
...
def get_window_title(hwnd: int) -> str:
    ...
    title_len = user.GetWindowTextLengthW(hwnd)
    if not title_len:
...
def get_window_rect(hwnd: int) -> Rect:
    ...
    """
    c_rect = wt.RECT()
    result = user.GetWindowRect(hwnd, ctypes.byref(c_rect))
    if not result:
...
def flash_window(
    ...
    user.FlashWindowEx(ctypes.byref(flash_info))
...
def set_active_window(hwnd: int):
    ...
    user.SetFocus(hwnd)# err 5
    user.BringWindowToTop(hwnd)# no err, but no result
    user.SetForegroundWindow(hwnd)# err 1400
    # user.SetActiveWindow(hwnd)# err 1400
...
def get_active_window_hwnd() -> int|None:
    ...
    """
    active_win_hwnd = user.GetForegroundWindow()
    if not active_win_hwnd or not user.IsWindowVisible(active_win_hwnd):
        return None
