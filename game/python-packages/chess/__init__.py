...
class Piece(object):
    ...
    def unicode_symbol(self, invert_colour=False):
        """
        Gets the Unicode character for the piece.
        """
        if not invert_colour:
            return UNICODE_PIECE_SYMBOLS[self.symbol()]
        else:
            return UNICODE_PIECE_SYMBOLS[self.symbol().swapcase()]

    def __hash__(self):
        return hash(self.piece_type * (self.colour + 1))
...
class BaseBoard(object):
    ...
    def pieces_mask(self, piece_type, colour):
        if piece_type == PAWN:
            bb = self.pawns
        ...
        return bb & self.occupied_co[colour]

    def pieces(self, piece_type, colour):
        """
        Gets pieces of the given type and colour.

        Returns a :class:`set of squares <chess.SquareSet>`.
        """
        return SquareSet(self.pieces_mask(piece_type, colour))
    ...
    def piece_at(self, square):
        ...
        if piece_type:
            ...
            colour = bool(self.occupied_co[WHITE] & mask)
    ...
    def king(self, colour):
        ...
        is no king of that colour.

        In variants with king promotions, only non-promoted kings are
        considered.
        """
        king_mask = self.occupied_co[colour] & self.kings & ~self.promoted
        if king_mask:
    ...
    def remove_piece_at(self, square):
        ...
        """
        colour = bool(self.occupied_co[WHITE] & BB_SQUARES[square])
        piece_type = self._remove_piece_at(square)
        if piece_type:
            return Piece(piece_type, colour)

    def _set_piece_at(self, square, piece_type, colour, promoted=False):
    ...
    def set_piece_at(self, square, piece, promoted=False):
        ...
        if piece is None:
            ...
            self._set_piece_at(square, piece.piece_type, piece.colour, promoted)
    ...
    def _set_board_fen(self, fen):
        ...
        for c in fen:
            if c in ["1", "2", "3", "4", "5", "6", "7", "8"]:
                ...
                self._set_piece_at(SQUARES_180[square_index], piece.piece_type, piece.colour)
                square_index += 1
            elif c == "~":
    ...
    def _set_piece_map(self, pieces):
        ...
        for square, piece in pieces.items():
            self._set_piece_at(square, piece.piece_type, piece.colour)
    ...
    def unicode(self, invert_colour=False, borders=False):
        builder = []
        for rank_index in range(7, -1, -1):
            ...
            for file_index in range(8):
                ...
                if piece:
                    builder.append(piece.unicode_symbol(invert_colour=invert_colour))
                else:
                    builder.append(u".")
...
class Board(BaseBoard):
    ...
    def _attackers_mask(self, colour, square, occupied):
        ...
        attackers = (
            ...
            (BB_PAWN_ATTACKS[not colour][square] & self.pawns))

        return attackers & self.occupied_co[colour]

    def attackers_mask(self, colour, square):
        return self._attackers_mask(colour, square, self.occupied)
    ...
    def is_attacked_by(self, colour, square):
        ...
        """
        return bool(self.attackers_mask(colour, square))

    def attackers(self, colour, square):
        """
        Gets a set of attackers of the given colour for the given square.

        Pinned pieces still count as attackers.

        Returns a :class:`set of squares <chess.SquareSet>`.
        """
        return SquareSet(self.attackers_mask(colour, square))
    ...
    def pin_mask(self, colour, square):
        king = self.king(colour)
        if king is None:
            return BB_ALL
        ...
        for attacks, sliders in [(BB_FILE_ATTACKS, self.rooks | self.queens),
            ...
            rays = attacks[king][0]
            if rays & square_mask:
                snipers = rays & sliders & self.occupied_co[not colour]
    ...
    def pin(self, colour, square):
        ...
        """
        return SquareSet(self.pin_mask(colour, square))

    def is_pinned(self, colour, square):
        """
        Detects if the given square is pinned to the king of the given colour.
        """
        return self.pin_mask(colour, square) != BB_ALL
    ...
    def castling_xfen(self):
        ...
        for colour in COLOURS:
            ...
            backrank = BB_RANK_1 if colour == WHITE else BB_RANK_8
            ...
            for rook_square in scan_reversed(self.clean_castling_rights() & backrank):
                ...
                other_rooks = self.occupied_co[colour] & self.rooks & backrank & ~BB_SQUARES[rook_square]
                ...
                builder.append(ch.upper() if colour == WHITE else ch)
    ...
    def _set_castling_fen(self, castling_fen):
        ...
        for flag in castling_fen:
            colour = WHITE if flag.isupper() else BLACK
            flag = flag.lower()
            backrank = BB_RANK_1 if colour == WHITE else BB_RANK_8
            rooks = self.occupied_co[colour] & self.rooks & backrank
            king = self.king(colour)
    ...
    def has_kingside_castling_rights(self, colour):
        ...
        """
        backrank = BB_RANK_1 if colour == WHITE else BB_RANK_8
        king_mask = self.kings & self.occupied_co[colour] & backrank & ~self.promoted
        if not king_mask:
            return False
    ...
    def status(self):
        ...
        # There must be exactly one king of each colour.
        ...
        # There can not be more than 16 pieces of any colour.
        ...
        # There can not be more than 8 pawns of any colour.
    ...
    def _valid_ep_square(self):
        if self.ep_square:
            ...
            # be a pawn of the correct colour on the fourth or fifth rank.
