...
CHECK_GRADIENT = """<radialGradient id="check_gradient"><stop offset="0%" stop-colour="rgba(255, 0, 0, 1)" /><stop offset="50%" stop-colour="rgba(231, 0, 0, 1)" /><stop offset="100%" stop-colour="rgba(158, 0, 0, 0)" /></radialGradient>"""
...
def board(board=None, squares=None, flipped=False, coordinates=True, lastmove=None, check=None, arrows=(), size=None, style=None):
    ...
    if board:
        for colour in chess.COLOURS:
            for piece_type in chess.PIECE_TYPES:
                if board.pieces_mask(piece_type, colour):
                    defs.append(ET.fromstring(PIECES[chess.Piece(piece_type, colour).symbol()]))
    if squares:
    ...
    for square, bb in enumerate(chess.BB_SQUARES):
        ...
        if lastmove and square in [lastmove.from_square, lastmove.to_square]:
            cls.append("lastmove")
        fill_colour = DEFAULT_COLOURS[" ".join(cls)]
        cls.append(chess.SQUARE_NAMES[square])

        # Fill colour is required for renderers without <style> support
        ...
        if board is not None:
            ...
            if piece:
                ET.SubElement(svg, "use", {
                    "xlink:href": "#%s-%s" % (chess.COLOUR_NAMES[piece.colour], chess.PIECE_NAMES[piece.piece_type]),
