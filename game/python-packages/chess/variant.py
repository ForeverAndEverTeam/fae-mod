...
class SuicideBoard(chess.Board):
    ...
    def is_insufficient_material(self):
        ...
        # Bishop and pawns of each side are on distinct colour complexes.
...
class AtomicBoard(chess.Board):
    ...
    def is_insufficient_material(self):
        ...
        if self.occupied == (self.kings | self.bishops):
            # All bishops on opposite colours.
...
class RacingKingsBoard(chess.Board):
    ...
    def status(self):
        ...
        if self.turn == chess.BLACK and all(self.occupied_co[co] & self.kings & chess.BB_RANK_8 for co in chess.COLOURS):
            status |= chess.STATUS_RACE_OVER
        if self.pawns:
            status |= chess.STATUS_RACE_MATERIAL
        for colour in chess.COLOURS:
            if chess.popcount(self.occupied_co[colour] & self.knights) > 2:
                status |= chess.STATUS_RACE_MATERIAL
            if chess.popcount(self.occupied_co[colour] & self.bishops) > 2:
                status |= chess.STATUS_RACE_MATERIAL
            if chess.popcount(self.occupied_co[colour] & self.rooks) > 2:
                status |= chess.STATUS_RACE_MATERIAL
            if chess.popcount(self.occupied_co[colour] & self.queens) > 1:
                status |= chess.STATUS_RACE_MATERIAL
        return status
