# TODO: Translation updated at 2018-07-28 13:12

# game/farewells.rpy:19
translate rus s_farewell_1_f3958667:

    # s "Are you quitting the game?"
    s "Ты уже уходишь?"

# game/farewells.rpy:5
translate rus s_farewell_1_c4ea05c6:

    # s 6abab "Then see you later, [player]!"
    s 6abab "Тогда удивимся позже, [player]!"

# game/farewells.rpy:10
translate rus s_farewell_2_9a1aefa3:

    # s "Goodbye, [player]!"
    s "Пока, [player]!"

# game/farewells.rpy:11
translate rus s_farewell_2_66d308ad:

    # s "Come back when you feel alone."
    s "Возврщайся, как заскучаешь."

# game/farewells.rpy:27
translate rus s_farewell_2_0d72d10b:

    # s 7aaca "I can always spend time with you."
    s 7aaca "Я всегда смогу провести с тобой время."

# game/farewells.rpy:17
translate rus s_farewell_3_aa6125ff:

    # s "Bye, honey!"
    s "Пока, дорогуша!"

# game/farewells.rpy:18
translate rus s_farewell_3_1a86ab7e:

    # s 7aaab "Don't forget to come back."
    s 7aaab "Не забудь только вернуться."

# TODO: Translation updated at 2018-07-30 10:22

# game/farewells.rpy:38
translate rus s_farewell_4_b4fdd37a:

    # s "Bye-bye!"
    s "Пока-пока!"


# game/farewells.rpy:44
translate rus s_farewell_5_a8da6f09:

    # s 7aaaa "Good night, [player]!"
    s 7aaaa "Спокойной ночи, [player]!"

# game/farewells.rpy:46
translate rus s_farewell_5_dcb8c0ac:

    # s 7acaa "You should sleep enough so you're in a good mood and not too tired."
    s 7acaa "Хороший сон — залог бодрости и хорошего настроения."

# game/farewells.rpy:47
translate rus s_farewell_5_c2b67bd3:

    # s 7aaaa "Have a good day, [player]!"
    s 7aaaa "Хорошего тебе дня, [player]!"

# game/farewells.rpy:49
translate rus s_farewell_5_f8caa2c4:

    # s 7aaaa "I hope you accomplish everything you planned for today."
    s 7aaaa "Пусть у тебя с успехом свершится всё на сегодня запланированное."

# game/farewells.rpy:50
translate rus s_farewell_5_c049ccaf:

    # s 7aaaa "See you later, [player]!"
    s 7aaaa "Увидимся ещё, [player]!"

# game/farewells.rpy:52
translate rus s_farewell_5_dc9d51c6:

    # s "I'm glad you have spent at least some of this evening with me."
    if gender:
        s "Я рада, что ты провёла хоть часть этого вечера со мной."
    else:
        s "Я рада, что ты провёл хоть часть этого вечера со мной."

# game/farewells.rpy:54
translate rus s_farewell_5_d542c3c8:

    # s "I'm glad you have spent this day with me."
    if gender:
        s "Я рада, что ты провёла этот день со мной."
    else:
        s "Я рада, что ты провёл этот день со мной."

# game/farewells.rpy:39
translate rus s_farewell_4_5d8035b0:

    # s "I wish you health and happiness."
    s "Счастья и здоровья тебе!"

# game/farewells.rpy:40
translate rus s_farewell_4_26b90d04:

    # s 7aaca "You can take some of mine, if you need it."
    s 7aaca "Можешь взять у меня немножко, если хочешь."
# TODO: Translation updated at 2018-08-21 15:27

# game/farewells.rpy:60
translate rus s_farewell_6_46bff501:

    # s "Bye, [palyer]!"
    s "Пока-пока, [palyer]!"

# game/farewells.rpy:61
translate rus s_farewell_6_8d5cd57e:

    # s "Take care of yourself."
    s "Береги себя."

# game/farewells.rpy:62
translate rus s_farewell_6_8dfd748f:

    # s "I want you to come back later safe and sound."
    s "Я хочу, чтобы ты потом вернулся целым и невредимым."

# game/farewells.rpy:67
translate rus s_farewell_7_dbfb08d4:

    # s "See you later, [palyer]!"
    s "До свидания, [palyer]!"

# game/farewells.rpy:68
translate rus s_farewell_7_85353668:

    # s "I wish you can give you a farewell kiss..."
    s "Жаль, что я не могу поцеловать тебя на прощание..."

# game/farewells.rpy:69
translate rus s_farewell_7_a2bffcdd:

    # s "But I think it's enough, that you know I'd do it."
    s "Хотя, думаю, что тебе достаточно лишь знать об этом."

# TODO: Translation updated at 2018-10-06 15:23

# game/farewells.rpy:60
translate rus s_farewell_6_6e992571:

    # s "Bye, [player]!"
    s "Пока-пока, [player]!"

# game/farewells.rpy:67
translate rus s_farewell_7_37bc89e5:

    # s "See you later, [player]!"
    s "Увидимся попозже, [player]!"

